# Frontend Documentation

## Product and integration

- [Main flows](MAIN_FLOWS.md)
- [API integration plan](API_INTEGRATION_PLAN.md)
- [Backend data route map](BACKEND_DATA_ROUTE_MAP.md)
- [Data alignment and mock removal plan](DATA_ALIGNMENT_AND_MOCK_REMOVAL_PLAN.md)
- [Betting backend contract](BETTING_BACKEND_CONTRACT.md)

## Feature plans

- [Owner, race registration, and jockey invitation flow](OWNER_RACE_JOCKEY_FLOW_PLAN.md)
- [Referee three-phase frontend plan](REFEREE_THREE_PHASE_FRONTEND_PLAN.md)
- [Spectator realtime betting and 2D race plan](SPECTATOR_REALTIME_BETTING_PLAN.md)
- [Reusable three-section 2D race renderer](THREE_SECTION_2D_RACE_RENDERER.md)
- [Spectator tournament and race redesign plan](SPECTATOR_TOURNAMENT_RACE_REDESIGN_PLAN.md)

## Design and assets

- [Frontend design document](design.md)
- [Jockey image assets](jockey-image-assets.md)
- [Horse owner image assets](owner-image-assets.md)

## Development references

- [Test accounts](TEST_ACCOUNTS.md)
- [Project guidance](CLAUDE.md)
