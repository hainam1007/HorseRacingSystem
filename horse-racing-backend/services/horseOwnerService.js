...
async function getProfile(user) {
  ...
  return {
    profile: Object.assign({}, owner.toObject ? owner.toObject() : owner, {
...
async function updateProfile(user, payload) {
  ...
  return {
    profile: Object.assign({}, updatedProfile.toObject ? updatedProfile.toObject() : updatedProfile, {
...
async function getAvailableJockeys(user, raceId) {
  ...
  const race = await horseOwnerRepository.findRaceById(raceId);

  if (!race) {
    throw new ApiError(404, 'Race not found');
  }

  const bindings = await horseOwnerRepository.findBoundJockeyIdsForRace(raceId);
...
async function getRacesByTournamentId(user, tournamentId) {
  ...
  const races = await horseOwnerRepository.findRacesByTournamentId(tournament._id);
  const counts = await horseOwnerRepository.countApprovedRegistrationsByRaceIds(races.map(function(race) {
    return race._id;
  }));
...
async function getRoundsByRaceId(user, raceId) {
  ...
  const round = await horseOwnerRepository.findRoundById(race.round_id);
...
async function createRegistration(user, owner, horseId, tournamentId, raceId, note, gears) {
  const [horse, tournament, race] = await Promise.all([
    ...
    horseOwnerRepository.findRaceById(raceId)
  ]);
  ...
  const existingRegistration = await horseOwnerRepository.findRegistrationByRaceAndHorse(race._id, horse._id);
  ...
  const initialCounts = await horseOwnerRepository.countApprovedRegistrationsByRaceIds([race._id]);
  ...
  if (!fee.entryFeeVnd) {
    ...
    try {
      registration = existingRegistration
        ? await horseOwnerRepository.updateRegistrationById(existingRegistration._id, { $set: noFeePayload })
        : await horseOwnerRepository.createRegistration(noFeePayload);
    } catch (error) {
  ...
  try {
    registration = existingRegistration
      ? await horseOwnerRepository.updateRegistrationById(existingRegistration._id, { $set: pendingPayload })
      : await horseOwnerRepository.createRegistration(pendingPayload);
  } catch (error) {
    await registrationSlotService.releaseRaceSlot(race._id);
    throw error;
  }
  ...
  try {
    ...
    await horseOwnerRepository.updateRegistrationById(registration._id, {
      $set: {
...
async function handleRegistrationPaymentWebhook(webhookPayload, paymentMethod) {
  ...
  const registration = await horseOwnerRepository.findRegistrationByPaymentOrderId(orderId);
  ...
  if (!paymentGatewayService.isPaymentSuccess(webhookPayload, paymentMethod)) {
    const failed = await horseOwnerRepository.updateRegistrationById(registration._id, {
      $set: {
        payment_status: 'failed',
        status: REGISTRATION_STATUS.REJECTED,
  ...
  if (reservationExpired || !registration.slot_reserved) {
    ...
    const refundPending = await horseOwnerRepository.updateRegistrationById(registration._id, {
      $set: {
  ...
  const confirmed = await horseOwnerRepository.updateRegistrationByPaymentOrderId(orderId, {
    $set: {
      payment_status: 'paid',
  ...
  if (!confirmed) {
    const latest = await horseOwnerRepository.findRegistrationByPaymentOrderId(orderId);
...
async function registerHorseForRace(user, payload) {
  const owner = await getCurrentOwner(user);
  const race = await horseOwnerRepository.findRaceById(payload.race_id);

  if (!race) {
    throw new ApiError(404, 'Race not found');
  }

  return createRegistration(user, owner, payload.horse_id, race.tournament_id, race._id, payload.note, payload.gears);
}

async function updateRaceEntryDetails(user, registrationId, payload) {
  const owner = await getCurrentOwner(user);
  const registration = await horseOwnerRepository.findRegistrationById(registrationId);
  ...
  const race = await horseOwnerRepository.findRaceById(registration.race_id);
  ...
  const updatedRegistration = await horseOwnerRepository.updateRegistrationById(registration._id, {
